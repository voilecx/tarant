...
An async [Tarantool](https://www.tarantool.io) client that speaks the binary
protocol natively, is typed end to end, and stays out of your way.
...
Interactive transactions require the server's MVCC transaction manager
...
name and no schema fetch is needed. Against 2.10–2.11 everything works except
name addressing. `Client::server_info()` reports what the handshake negotiated.
